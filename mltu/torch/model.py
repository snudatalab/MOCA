...
class Model:
    ...
    def generate_pseudo_labels(self, dataProvider):
        """Generate pseudo-labels for each instance in dataProvider."""
        ...
        pseudo_labels = {}
        with torch.no_grad():
            for idx, (data, target) in enumerate(dataProvider):  # Track the index of each batch
                data = torch.from_numpy(data).float().to(self._device)
                output = self.model(data)
                argmax_preds = torch.argmax(output, dim=-1).cpu().numpy()
                for preds in argmax_preds:
                    decoded_sequence = np.array([[int(k) for k, _ in groupby(preds)]], dtype=int)
                    pseudo_labels[idx] = decoded_sequence  # Store decoded sequence by index instead of data tensor
    ...
    def train(self, dataProvider: DataProvider):
        ...
        # pseudo_labels = self.generate_pseudo_labels(dataProvider)
        ...
        for step, (data, target) in enumerate(pbar, start=1):
            ...
            tmp_step = step - cnt
            # target = torch.tensor(pseudo_labels[step - 1]).to(self._device)  # Assuming step - 1 aligns with idx in dataProvider

            # if (data.shape[1])<150000 and (target.shape[1]>0) and (torch.any(target != 28)) :
            if (data.shape[1])<150000:

                data, target = self.toDevice(*toTorch(data, target))
                loss = self.train_step(data, target)
                loss_sum += loss.item()
                loss_mean = loss_sum / tmp_step
                ...
                cnt += 1
                print("Number of Ignored Instances : ",cnt)
